...
data = scipy.io.loadmat('/home/eunsongk/Data/MDD/MDD_84.mat')["Data"]
labels = scipy.io.loadmat('/home/eunsongk/Data/MDD/MDD_84.mat')["Labels"]
# print(data.shape)
# (62,1)
...
# data = scipy.io.loadmat('/home/eunsongk/Data/MDD/MDD_136*170*116.mat')["Data"]
# labels = scipy.io.loadmat('/home/eunsongk/Data/MDD/MDD_136*170*116.mat')["Labels"]
# #(136, 170, 116)
# labels = np.array([labels] * 170).transpose()
# labels = np.asarray(labels).ravel()
# labels = np.asarray(labels)
...
flags.DEFINE_integer('batch_size', 170, 'Minibatch size')
...
def train():
    ...
    with tf.variable_scope('model', reuse=True):
        ...
        for i in range(FLAGS.n_iterations * (n_samples // FLAGS.batch_size)):
            ...
            if i == (FLAGS.n_iterations * (n_samples // FLAGS.batch_size) - FLAGS.print_every):
            ...
            # if offset==0 and i!=0:t
                ...
                X = sess.run(q_mu, {x: np_x_all})
                labels_tsne = np.argmax(np_y_all, 1)

                Y = tsne_MDD.tsne(X, 2, 35, 50.0)
...
#################################################################Original value was 20 not 35####################################################
                if labels_tsne[0] == 1:
                  plt.scatter(Y[:, 0], Y[:, 1], 35, c = labels_tsne, cmap=mpl.colors.ListedColormap('red'))
                else:
                  plt.scatter(Y[:, 0], Y[:, 1], 35, c=labels_tsne, cmap=mpl.colors.ListedColormap('blue'))
