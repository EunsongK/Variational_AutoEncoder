...
flags.DEFINE_integer('batch_size', 260, 'Minibatch size')
flags.DEFINE_integer('n_samples', 50, 'Number of samples to save')
...
def train():
    ...
    with tf.variable_scope('model', reuse=True):
        ...
        # Get fixed x and y for plotting
        np_x_fixed = arr2
        np_y_fixed = test_label
        ...
        for i in range(FLAGS.n_iterations * (n_samples // FLAGS.batch_size)):
            ...
            # np_y = training_label[offset * FLAGS.batch_size:(offset + 1) * FLAGS.batch_size].reshape(-1, 2)
            # np_y_tsne = training_label[offset * FLAGS.batch_size:(offset + 1) * FLAGS.batch_size]
